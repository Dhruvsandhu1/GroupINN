...
class data_manager:
    def __init__(self, args, train_data_repeat=1, reg_selected_features=None):
        ...
        self.training_shuffle_randint = (
            lambda: self.training_shuffle_random.randint(0, 2**31 - 1))
        ...
        self.train_filenames = self.meta_table[train_mask].SubID  # noqa: disable=E501
        self.test_filenames = self.meta_table[test_mask].SubID  # noqa: disable=E501
    ...
    def mat_file_selector(self, prev_train_mask, prev_test_mask):
        ...
        subjectID_list = self.meta_table.SubID
    ...
    def gen_validation_set(self):
        print("Training set size before validation split:", len(self.train_filenames))
        if self.args.validation_proportion:
            if len(self.train_filenames) == 0:
                raise ValueError("No training samples available after filtering. Check missing data handling.")
...
class corr_manager(data_manager):
    def gen_subject_tensor(self, subjectID):
        def correlation(time_series):
            ...
            time_series=time_series['wmLR'] if 'wmLR' in time_series.keys() else time_series['wmRL']
...
class corr_mean_manager(data_manager):
    def gen_subject_tensor(self, subjectID):
        ...
        if subjectID in cache:
            ...
            mat_data = {key: self.mat_file_loader(subjectID, filename) for key, filename in self.args.load_mat_dict.items()
                if self.mat_file_checker(subjectID, filename)}
            # for key, timeseries in mat_data.items():
            #     print("#######################################################")
            #     print(timeseries.keys())
            #     print(correlation(timeseries['wmLR']))
            #     print("#######################################################")
            corr_data = {key: correlation(timeseries) for key, timeseries in mat_data.items()}
